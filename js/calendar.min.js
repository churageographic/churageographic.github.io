'use strict';const peakBorder=35,calendarDateFrom="2021-01-01",calendarDateTo="2025-12-31";async function getDataAndDrawCalendar(){const c=(new Date).getFullYear(),b=$("#area option:selected").val(),d=b+c;null!=sessionStorage.getItem(d)?drawCalendar(JSON.parse(sessionStorage.getItem(d))):await getJsonData(c,b).then(a=>{a=createEvents(a);sessionStorage.setItem(d,JSON.stringify(a));drawCalendar(a)},a=>{for(let g in a)a.warn?toastr.warning(a.warn):toastr.error(a.error)})}
function getJsonData(c,b){return new Promise((d,a)=>{const g=b.substr(0,2),f=b.substr(2);$.get(`https://churageographic.github.io/data/${g}/${f}/${c}.json`,function(e){e.warn||e.error?a(e):d(e)},"json").fail(function(e){toastr.error(`\u300c ${$("#area option:selected").text()}\u306e\u60c5\u5831\u3092\u53d6\u5f97\u3067\u304d\u307e\u305b\u3093\u3067\u3057\u305f`)})})}
function createEvents(c){let b=[];for(let a=0;a<c.length;a++){var d=c[a].l;if(d>=peakBorder)continue;const g=String(c[a].t).padStart(2,"0");d={id:a+1,title:d+"cm",start:c[a].d+" "+g+":00",backgroundColor:getTidesColor(d)};b.push(d)}return b}function getTidesColor(c){return 0>=c?"#ff1493":15>=c?"#ff8c00":c<=peakBorder?"#228b22":"#15A0C8"}function getCalendarHeight(){return window.innerHeight-100}var calendar,tmpY=(new Date).getFullYear();
...
{let d=(new Date((b.start.getTime()+b.end.getTime())/2)).getFullYear();if(tmpY!=d){tmpY=d;const a=$("#area option:selected").val(),g=a+d;null==sessionStorage.getItem(g)&&await getJsonData(d,a).then(f=>{let e=createEvents(f);sessionStorage.setItem(g,null);b.view.calendar.batchRendering(function(){e.forEach(async h=>{await b.view.calendar.addEvent(h)})})},f=>{for(let e in f)f.warn?toastr.warning(f.warn):toastr.error(f.error)})}}})).render()}
...
g){var f=$("<optgroup>").prop("label",a);Object.keys(this[a]).forEach(function(e,h){f.append($("<option>").text(this[e].text).val(this[e].value).prop("selected","4704"==this[e].value))},this[a]);d.push(f)},c);b.append(d);b.change(function(){$(".fc-areaList-button").text($("#area option:selected").text());$("#areaDialog").dialog("close");getDataAndDrawCalendar()});$(document).ready(function(){b.select2()});$(document).on("click",".ui-widget-overlay",function(){$(this).prev().find(".ui-dialog-content").dialog("close")})}
